	...
	for(i=0; i<str1size/block_x; i++)
	{
		...
		for(j=0; j<str2size/block_y; j++)
		{
			...
			result = calculate_submatrix(top[j], left, diag[i][j],
					substr1[i], substr2[j], block_x, block_y); 
			...
			{	
				if(i<str1size/block_y-1) //Have not reached bottom.
				{
...
int** calculate_submatrix(int* top, int* left, int diag, char* str1, char* str2, int str1size, int str2size)
{
